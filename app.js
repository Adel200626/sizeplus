...
var ARTICLES_SECOURS = [
    {
        id:0, name:'Haut Noir',
        tailles:['44','48'],
        prix:3000, ancien:4600, reduc:35,
...
function loadProducts() {
    ...
    try {
        ...
        if (cached) {
            showArticles(JSON.parse(cached));
        } else {
            // Pas de cache → logo centré
            document.getElementById('resultsCount').textContent = '';
            document.getElementById('productsGrid').innerHTML =
                '<div style="text-align:center;padding:80px 20px">' +
                '<div style="width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,#c2185b,#e91e8c);color:#fff;font-size:32px;font-weight:700;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-family:Poppins,sans-serif;box-shadow:0 4px 20px rgba(194,24,91,.35)">S</div>' +
                '<div style="font-size:22px;font-weight:700;color:#c2185b;font-family:Poppins,sans-serif;letter-spacing:2px">SAMEL</div>' +
                '<div style="font-size:12px;color:#bbb;margin-top:6px;font-family:Poppins,sans-serif">Mode Femme · Grande Taille</div>' +
                '</div>';
        }
    } catch(e) {}

    // 2. Charger depuis le Sheet en arrière-plan (mise à jour silencieuse)
    loadFromSheet();
}

// ============================================================
//  Chargement via Google Apps Script (fiable, pas de CORS)
// ============================================================
var GAS_URL = 'https://script.google.com/macros/s/AKfycbz9Bcl8Y87jiL44LWb18lWupN1ehilXPAK15fQAdnyxrKpvb7nkChfpYUgl3uvKcBsX/exec';

function showArticles(list) {
    promoProducts = list;
    renderProducts(promoProducts);
    buildFilters();
    renderSideTicker(promoProducts);
}

function loadFromSheet() {
    var fallbackTimer = setTimeout(function() {
        if (!promoProducts.length) showArticles(ARTICLES_SECOURS);
    }, 8000);
    ...
    fetch(GAS_URL + '?t=' + Date.now())
        ...
        .then(function(articles) {
            ...
            var list = (articles && articles.length > 0) ? articles : ARTICLES_SECOURS;
...
// ============================================================
function renderSideTicker(list) {
    var inner = document.getElementById('sideTickerInner');
    if (!inner || !list.length) return;
    var html = '';
    // Doubler pour l'effet infini
    for (var pass = 0; pass < 2; pass++) {
        for (var i = 0; i < list.length; i++) {
            var p = list[i];
            html += '<div class="side-ticker-item" onclick="openModal(' + p.id + ')">' +
                '<img src="' + esc(p.img) + '" alt="' + esc(p.name) + '" onerror="this.style.opacity=\'0\'">' +
                '<div class="side-ticker-item-name">' + esc(p.name) + '</div>' +
                '<div class="side-ticker-item-old">' + fmt(p.ancien) + '</div>' +
                '<div class="side-ticker-item-price">' + fmt(p.prix) + '</div>' +
            '</div>';
        }
    }
...
function cardHTML(p) {
    ...
    return '<div class="product-card promo-card" data-id="' + p.id + '">' +
        ...
        '<div class="card-body">' +
            ...
            priceTag +
            sizesTag +
            '<div class="stock-limite">⚡ Stock limité</div>' +
        '</div>' +
...
function sendOrder() {
    ...
    for (var i = 0; i < cart.length; i++) {
        var item = cart[i];
        var ref = item.product.img ? item.product.img.split('/').pop().replace(/\.[^.]+$/, '') : '';
        lines.push((i+1) + '. ' + item.product.name + (ref ? ' (Réf. ' + ref + ')' : '') + ' | Taille: ' + item.taille + ' | Qté: ' + item.qty + ' | ' + fmt(item.product.prix * item.qty) + ' 🔥');
...
// ============================================================
//  Compte à rebours — date de fin des soldes
// ============================================================
var SALE_END = new Date('2026-04-20T23:59:59');
