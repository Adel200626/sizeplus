...
// ============================================================
//  Chargement Google Sheets via JSONP
// ============================================================
function loadFromSheet() {
    ...
    var timer = setTimeout(function() {
        ...
        // Timeout = garder les articles de secours (déjà affichés)
    }, 15000);
    ...
    window[cb] = function(data) {
        ...
        if (scr && scr.parentNode) scr.remove();
        try {
            var articles = parseSheet(data);
            if (articles.length > 0) {
                // Mise à jour réussie depuis Google Sheets
                promoProducts = articles;
                renderProducts(promoProducts);
                buildFilters();
            }
            // Si 0 articles depuis le sheet → garder les articles de secours
        } catch(e) {
            // Erreur de parsing → garder les articles de secours
        }
    };
    ...
    scr.onerror = function() {
        clearTimeout(timer);
        delete window[cb];
        // Erreur réseau → garder les articles de secours (déjà affichés)
    };
...
// ============================================================
//  Parsing des données Google Sheets
// ============================================================
